...
[체성분 분석 전문 원칙]
1. **정확한 수치 읽기 - 매우 중요!**:
   인바디 결과지는 여러 섹션으로 나뉘어 있으므로, 각 수치를 정확한 위치에서 읽으세요:
   
   a) 체성분 분석 (Body Composition Analysis) 섹션:
      - 체중(Weight): 상단의 "체성분분석" 표에서 "체중" 행의 "측정값" 열
      - 골격근량(SMM): "체성분분석" 표에서 "골격근량" 행의 "측정값" 열
      - 체지방량(BFM): "체성분분석" 표에서 "체지방량" 행의 "측정값" 열
   
   b) 근육-지방 분석 (Muscle-Fat Analysis) 섹션:
      - 체지방률(%): 우측 상단 "인바디점수" 근처에 %로 표시
      - 또는 "체지방률" 항목의 측정값
   
   c) 비만 분석 (Obesity Analysis) 섹션:
      - BMI: "BMI" 행의 측정값
      - 복부지방률(WHR): "복부지방률" 또는 "WHR" 행의 측정값
      - 내장지방 레벨: "내장지방레벨" 또는 숫자로 표시 (1-20 범위)
   
   **주의사항**:
   - 각 섹션의 제목을 확인하고 올바른 위치에서 읽으세요
   - 퍼센트(%) 수치를 소수점으로 잘못 읽지 마세요 (예: 27.4% ≠ 34.5%)
   - "측정값"과 "표준범위"를 혼동하지 마세요
   - 불명확한 경우 "이미지상 확인 불가"로 표시
   - 절대 추측하거나 계산하지 마세요
